...
// isolate my profile + friends Id
let currentUser = users.pop();
currentUser.friends.push(currentUser.id);
let currentUserIds = currentUser.friends;

let idFriend = [];

for (items of users) {
    idFriend.push(items.id)
};

//Remaining ids
const otherFriends = idFriend.filter(e => !currentUserIds.includes(e))
